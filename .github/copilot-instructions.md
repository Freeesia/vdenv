...
`vdenv` はWindows専用の .NET 8 dotnet toolで、Windowsの仮想デスクトップごとに環境変数とシェルプロファイルを切り替えます。実行すると一時バッチファイルを生成してそのパスを標準出力に書き出し、呼び出し元（`cmd.exe /k for /f ...`）がそのバッチファイルを実行することでシェルに設定を適用します。
